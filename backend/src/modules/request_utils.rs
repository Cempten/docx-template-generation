...
pub fn get_boundary(cont_type: &ContentType) -> Result<String, String> {
    let (_, boundary) = match cont_type.params().find(|&(k, _)| k == "boundary") {
        Some(x) => x,
        None => return Err(String::from("Boundary param not provided")),
    };
    Ok(String::from(boundary))
}

pub fn get_file_from_req_body(entries: &Entries) -> Result<(String, PathBuf), String> {
    let file = match entries.fields.get("file") {
        Some(file) => file,
        None => return Err(String::from("File is missing")),
    };
    ...
    Err(String::from("Failed to read the contents of the file"))
}
