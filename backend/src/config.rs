...
use crate::modules::{placeholders, request_utils, templates};
...
#[get("/template")]
pub fn get_template() -> Json<Vec<String>> {
    let templates_names = templates::get_templates_names();
    Json(templates_names)
}

#[post("/template", data = "<data>")]
pub fn new_template(cont_type: &ContentType, data: Data) -> Result<Json<String>, Status> {
...
pub fn get_placeholders(name: String) -> Result<Json<Vec<String>>, Status> {
...
pub fn get_filled_template(data: Json<Placeholders>, name: String) -> Result<Json<String>, Status> {
...
#[get("/template/<name>")]
pub fn download_template(name: String) -> Result<FileStruct, Status> {
...
pub fn delete_template(name: String) -> Result<Json<String>, Status> {
    let template_name = match templates::try_to_find_template(name) {
        Ok(template_name) => template_name,
        Err(_) => return Err(Status::NotFound),
    };
...
#[catch(404)]
pub fn not_found() -> Json<CustomError> {
    Json(CustomError {
        status: 404,
